 ...
 */
class CRM_Searchtaskbuilder_Form_ManageSearchTasks extends CRM_Core_Form {
  ...
  public function preProcess() {
    ...
    $searchTasks = array();
    $dao = CRM_Core_DAO::executeQuery($sql, array(), false, 'CRM_Searchtaskbuilder_DAO_SearchTask');
    while($dao->fetch()) {
      $row = array();
      CRM_Core_DAO::storeValues($dao, $row);
      switch ($row['status']) {
        case CRM_Searchtaskbuilder_Status::IN_CODE:
          $row['status_label'] = E::ts('In code');
          break;
        case CRM_Searchtaskbuilder_Status::OVERRIDDEN:
          $row['status_label'] = E::ts('Overridden');
          break;
        case CRM_Searchtaskbuilder_Status::IN_DATABASE:
