  ...
  public static function getUnmetDependencyErrorMessage($unmet) {
    switch ($unmet) {
      case 'action-provider':
        return ts('Search Action Designer was installed successfully, but you must also install and enable the <a href="%1">action-provider Extension (version 1.36 or newer)</a>.', array(1 => 'https://lab.civicrm.org/extensions/action-provider'));
  ...
  public static function checkExtensionDependencies() {
    ...
    foreach($extensions['values'] as $ext) {
      if ($ext['key'] == 'action-provider' && $ext['status'] == 'installed') {
        if (version_compare($ext['version'], '1.36', '>=')) {
