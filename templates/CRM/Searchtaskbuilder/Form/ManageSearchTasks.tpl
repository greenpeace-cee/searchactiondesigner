{crmScope extensionKey='searchtaskbuilder'}
  ...
  <div class="crm-content-block">
    ...
    <div class="action-link">
      <a class="button" href="{crmURL p="civicrm/searchtaskbuilder/edit" q="reset=1&action=add" }">
        <i class="crm-i fa-plus-circle">&nbsp;</i>
        {ts}Add Search Task{/ts}
      </a>
      <a class="button" href="{crmURL p="civicrm/searchtaskbuilder/import" q="reset=1&action=add" }">
          ...
          {foreach from=$search_tasks item=search_task}
            <tr>
              ...
              {if $search_task.is_active eq 1}
                <td><span><a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=disable&id=`$search_task.id`"}"
                             class="" title="{ts}Disable Search Task{/ts}">{ts}Enabled{/ts}</a></span></td>
              {else}
                <td><span><a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=enable&id=`$search_task.id`"}"
              ...
              <td>
                ...
                {if ($search_task.status eq 3)}
                  <span>
                      <a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=revert&id=`$search_task.id`"}"  class="" title="{ts}Revert Search Task{/ts}">
                          {ts}Revert{/ts}
                      </a>
                  </span>
                {/if}
              ...
              <td>
                  <span>
                  <a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=update&id=`$search_task.id`"}"
                     class="action-item crm-hover-button" title="{ts}Edit Search Task{/ts}">{ts}Edit{/ts}</a>
                  <a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=export&id=`$search_task.id`"}"
                     class="action-item crm-hover-button" title="{ts}Export Search Task{/ts}">{ts}Export{/ts}</a>
                  <a href="{crmURL p='civicrm/searchtaskbuilder/edit' q="reset=1&action=delete&id=`$search_task.id`"}"
