        ...
        <table>
            ...
            {foreach from=$fields item=field}
                ...
                <tr>
                    ...
                    <td>{$field.weight}</td>
                    <td>
                        <a href="{crmURL p="civicrm/searchtaskbuilder/field" q="reset=1&action=update&search_task_id=`$field.search_task_id`&id=`$field.id`"}">{ts}Edit{/ts}</a>
                        <a href="{crmURL p="civicrm/searchtaskbuilder/field" q="reset=1&action=delete&search_task_id=`$field.search_task_id`&id=`$field.id`"}">{ts}Remove{/ts}</a>
                    </td>
                </tr>
            {/foreach}
        </table>
